...
def main(opt):
    ...
    if bool(opt.van) == True:
        #before_HS_list, post4h_list, post24h_list = get_data_at_timepoint(data_df, hss, hsd)
        #plot_timepoint_hist([before_HS_list, post4h_list, post24h_list], plot_dir, opt)
        #### histogram
        df_list = df_Processing_HS(data_df, hss,hsd, end_time, opt)
        HSPR_list = bootHSPR_toList(df_list)
        bootstrap_HSPR_hist_overlap(HSPR_list, plot_dir, opt)
        exit()
        #violin_plot(HSPR_list, plot_dir, opt)
        ### boxplot
        snapshot_list = get_snapshot_conclist(data_df, hss, hsd)
        #box_plot(snapshot_list, plot_dir, opt)
        violin_plot(snapshot_list, plot_dir, opt)
...
def load_Param_fromDataFile(param_rootdir, data_dir, partiii_dir, varPara_dir, varData_dir, opt):
    if bool(re.search(re.compile(r'/'), opt.ids)) == False:
        ...
        S_val = float(re.match(r'^(-?[\d.]+)_', filename).group(1))
        #except AttributeError: S_val = float(re.match(r'^([\d.]+)_', filename).group(1))
...
def param_dict_import_impute(para_csv_name, model_name, opt):
    ...
    for key, val in zip(headers, data):
        if key == 'model_name': 
            ...
            try: param_dict[key] = float(val)
            except ValueError: param_dict[key] = val
...
def plot_trajectory(opt, ax, data_df, x_col, y_col_list, hss, hsd, Iteration_Identifier):
    ...
    for i, y_col in enumerate(sorted(y_col_list)):
        ax.plot(data_df[x_col], data_df[y_col], color = color_palette[i], label=y_col, linewidth=1, alpha=0.8)
    ax.set_xlabel('Time (hour)', fontsize = 16)
    ax.set_ylabel('Protein copy number', fontsize = 16)
    ...
    ax.tick_params(axis='x', labelsize=14)  # Set x-axis tick label fontsize to 12
...
def get_data_at_timepoint(data_df, hss, hsd):
    before_HS_list = data_df[(data_df['time'] <= hss) & (data_df['time'] >= hss-5)]['totalHSPR'].tolist()
    before_HS_list2 = data_df[data_df['time'] == hss-50]['totalHSPR'].tolist()
    post4h_list = data_df[(data_df['time'] <= hss+hsd+4 +2.5) & (data_df['time'] >= hss+hsd+4 -2.5) ]['totalHSPR'].tolist()
    post24h_list = data_df[(data_df['time'] <= hss+hsd+24 +2.5) & (data_df['time'] >= hss+hsd+24 -2.5) ]['totalHSPR'].tolist()

    before_HS_list = data_df[data_df['time'] == hss]['totalHSPR'].tolist()
    post4h_list = data_df[data_df['time'] == hss+hsd+4]['totalHSPR'].tolist()
    post24h_list = data_df[data_df['time'] == hss+hsd+24 ]['totalHSPR'].tolist()
    #print('before_HS_list', before_HS_list)
    #print('post4h_list', post4h_list)
    #print('post24h_list', post24h_list)
    return before_HS_list, post4h_list, post24h_list

def plot_timepoint_hist(data_list, plot_dir, opt):
    label_list = ["before HS", "4h post HS", "24h post HS"]
    empty_list, cv_list, description = [], [], ''
    for list, label in zip(data_list, label_list):
        empty_list += list
        sd = np.std(list)
        mean = np.mean(list)
        CV = sd/mean
        cv_list.append(CV)
        description += f'CV {label} = {"{:.4f}".format(CV)}\n'
    maxVal = int(max(empty_list))
    print(description)
    print("Plot time point HSPR histogram")
    fig = plt.figure(figsize=(12, 6))
    plt.hist(data_list, bins = range(0,maxVal, 1), label = label_list, density=True, alpha=0.50, histtype='stepfilled')
    plt.title("Distribution of HSPR conc, at three time points")
    plt.xlabel("HSPR")
    plt.ylabel("Frequency")
    #plt.text(0.95 ,0.95, description, fontsize = 9, verticalalignment='top',horizontalalignment='right')
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.) # Adjust the figure size to accommodate the legend
    plt.subplots_adjust(right=0.8)  # Increase the right margin
    plt.tight_layout()

    saveFig(plot_dir, str(opt.name_suffix), opt, prefix ='timepointHist')
    if bool(opt.shf) == True: plt.show()
    plt.close()
...
def bootHSPR_toList(df_list):
    HSPR_list = []
    samp_size = int(0.7*min(len(df['totalHSPR'].tolist()) for df in df_list))
    #sample_size = 100
    print('bootstrap sample size = ', samp_size)
    for df in df_list:
        HSPR_conc = random.choices(df['totalHSPR'].tolist(), k=samp_size)
        HSPR_list.append(HSPR_conc)
    return HSPR_list



def bootstrap_HSPR_hist_overlap(data_list, plot_dir, opt):
    ...
    plt.hist(data_list, bins = range(0, maxVal, 1), label = label_list, density=True, alpha=0.50, histtype='stepfilled')
    plt.title(f"Model {opt.name_suffix[:3]}", fontsize = 18)
    plt.xlabel("total HSPR", fontsize = 16)
    plt.ylabel("Frequency", fontsize = 16)
    plt.tick_params(axis='x', labelsize=14)
    plt.tick_params(axis='y', labelsize=14)
    #plt.text(0.9 ,0.9, description, fontsize = 9, verticalalignment='top')
    plt.legend(loc='upper right', borderaxespad=0., fontsize = 16,handlelength=4, handleheight=2) # Adjust the figure size to accommodate the legend
    plt.subplots_adjust(right=0.8)  # Increase the right margin
    plt.tight_layout()

    saveFig(plot_dir, f"Model-{opt.name_suffix[:3]}", opt, prefix ='HSperiod_hist')
    if bool(opt.shf) == True: plt.show()
    plt.close()
...
def get_snapshot_conclist(data_df, hss, hsd):
    preHS = data_df[data_df['time'] == hss-0.1]['totalHSPR']
    print('preHS', len(preHS))
    post_4h = data_df[data_df['time'] == hss+hsd+4]['totalHSPR']
    print('post_4h', len(post_4h))
    post_24h = data_df[data_df['time'] == hss+hsd+24]['totalHSPR']
    print('post_24h', len(post_24h))
    snapshot_list = [preHS, post_4h, post_24h]
    return snapshot_list



def box_plot(snapshot_list, plot_dir, opt):
    # Create a box plot
    #plt.boxplot(snapshot_list)

    sns.boxplot(data=snapshot_list, showfliers=True)  # hide outliers
    #for i, data in enumerate(snapshot_list):
    #    sns.stripplot(x = [i]*len(data), data=data, color='red', jitter=True, alpha=0.3)  # add dots

    # Add labels to the x-axis
    plt.xticks([0, 1, 2], ['preHS', '4h after', '24h after'])
    # Add a title and labels to the axes
    #plt.title('Box Plot of Three Datasets')
    plt.ylabel('total HSPR in each iteration')

    saveFig(plot_dir, str(opt.name_suffix), opt, prefix ='boxplot')
    if bool(opt.shf) == True: plt.show()
    plt.close()


def violin_plot(snapshot_list, plot_dir, opt):
    label_list = ['preHS', '4h after', '24h after']
    empty_list, cv_list, description = [], [], ''
    for list, label in zip(snapshot_list, label_list):
        sd = np.std(list)
        mean = np.mean(list)
        CV = sd/mean
        cv_list.append(CV)
    ...
    plt.violinplot(dataset=snapshot_list, showmeans=True)
    plt.xticks([1, 2, 3], ['preHS', '4h after', '24h after'], fontsize = 16)
    plt.tick_params(axis='y', labelsize=14) 
    #plt.xlabel('model behaviour')
    plt.ylabel('total HSPR in each iteration', fontsize = 16)
    #plt.grid(True)
    plt.title(f"Model {opt.name_suffix[:3]}", fontsize = 18)

    plt.tight_layout()
    saveFig(plot_dir, f"Model-{opt.name_suffix[:3]}", opt, prefix ='violinplot')
    if bool(opt.shf) == True: plt.show()
    plt.close()
